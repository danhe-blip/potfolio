...
- **New facts (Aug 2026, from Dan):** (1) Early on, the product was demonstrated to a major enterprise retailer, got strong feedback, and advanced the commercial conversation. Safe if kept anonymous. Deliberately NOT in copy for now: the case already claims flagship plus enterprise production use, and an early promising demo reads weaker beside that; available if an early-validation beat is ever wanted. (2) The atomic product object is confirmed as Rule, so the shipped composition language (rules → policy groups → guard) is correct. Do not rename to "Rules → Policies → Guard" (outside advice, rejected: it would contradict the product UI in the screenshots). (3) "Whoever opened that boundary first would own the category" softened to "No product on the market had opened that boundary yet. That was the opening." (market-conquest phrasing, per Dan).
...
REFINED (Aug 2026, lean pass, same framework as AI Workflow and Design System). Seven beats became six: Next folded into Contribution. This was a refine, not a recut; the case was already decision-led. What changed: 02 Focus and 03 Problem deduplicated (thresholds/eval-sets and the locked-out policy owner each said once, Focus carries the bet, Problem carries the concrete gap); the "what I would move earlier" retrospective moved from Next to the close of 05 Outcome, killing the double-told onboarding story; 06 Contribution now uses the `rules` field (The abstraction / The component / The two-day loop, "Touch the guard, not the policy" restored from the archive), with the founding-design principle and the next-hire test as the coda, keeping the design-system link and the query-table image; Context's closing run-on split into two sentences. 04 The Build untouched on purpose: three paragraphs, three claims, the tension arc, already the standard. All fact rules intact: "profitable" once in Context, persona axis correct, no customer names. Pre-refine copy recoverable from git history (commit 0de22a8, main). Note: the "full copy (archive)" section below predates the Aug 2026 revision and this pass; restore from git history, not from the archive.
...
GATED (Aug 2026): like PolicyGuard, the case is public through 01 Context plus the Result callout, then a name gate. Gated sections live in api/_gated.js under "design-system". Confidential flag removed; the cell is clickable.

RECUT (Aug 2026, lean pass, same framework as AI Workflow): eight beats became six. 01 Context (untouched, public) / 02 The Call (Audit merged in as the call's evidence; the shadcn trade-off now said out loud: out of the box we looked like every other AI product, and that was the point) / 03 The Build (headline promoted from label to claim, "The distance between stock and ours is the design work"; restraint stated per the research framing: "What cleared the bar got systematized. What didn't, deliberately didn't.") / 04 In Product / 05 Source of Truth (gained a sec-link to ai-workflow, "The same inversion built this site") / 06 Outcome (Next folded in, ends on the PolicyGuard link). Cut on purpose: "It did.", "That is what treating the system as a product means" (restated the headline), the standalone Audit and Next beats. Result callout now leads with day-one inheritance ("the third will inherit them on day one", confirmed fact: the agent guardrail is product three). All four ds images and the layers diagram unchanged. Pre-recut copy recoverable from git history (commit 9ecc2ce, main).
